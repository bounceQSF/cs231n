...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type == 'rnn':
            dcap, dh0, grads['Wx'], grads['Wh'], grads['b'] = rnn_backward(dh, rnn_cache)
        elif self.cell_type == 'lstm':
            dcap, dh0, grads['Wx'], grads['Wh'], grads['b'] = lstm_backward(dh, rnn_cache)
        dfeatures, grads['W_proj'], grads['b_proj'] = affine_backward(dh0, h0_cache)
        grads['W_embed'] = word_embedding_backward(dcap, embed_cache)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def sample(self, features, max_length=30):
        ...
        for i in range(1, max_length):
            ...
            if self.cell_type == 'rnn':
                h, rnn_cache = rnn_step_forward(x_embed, h, Wx, Wh, b)
            elif self.cell_type == 'lstm':
                h, c, rnn_cache = lstm_step_forward(x_embed, h, c, Wx, Wh, b)

            scores, out_cache = affine_forward(h, W_vocab, b_vocab)

...
class TwoLayerNet(object):
  ...
  """
    ...
    """
        ...
        delta1 = prob
        delta1[np.arange(N), y] -= 1

        grads['b2'] = np.sum(delta1, axis=0) / N
        grads['W2'] = np.dot(h.T, delta1) / N

        delta2 = delta1.dot(W2.T)  # N * H
        delta2[h == 0] = 0
        grads['b1'] = np.sum(delta2, axis=0) / N
        grads['W1'] = np.dot(X.T, delta2) / N
    ...
    """
        ...
        for it in range(num_iters):
            ...
            #########################################################################
            for item in grads:
                self.params[item] -= learning_rate * grads[item]
            #########################################################################
            #                             END OF YOUR CODE                          #
            #########################################################################
